...
public class JavaIDE3 extends JFrame {
    ...
    ArrayList<JTextArea> file_jtextarea_list = new ArrayList<JTextArea>();
    ...
    private JTabbedPane createTabbedPane(String file_name, String file_path, String file_content) {
        // resultArea.setText("asdf");
        // 탭 추가
        JTextArea editor = new JTextArea(file_content);
        JScrollPane scrollPane = new JScrollPane(editor);
        scrollPane.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_ALWAYS);
        scrollPane.setHorizontalScrollBarPolicy(JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
        // addTab(파일 이름, 파일 내용) <- fildchooser에서 가져온 파일정보를 따서 넣으면 되지 않을까
        tabbedPane.addTab(file_name, scrollPane);
        file_jtextarea_list.add(editor);
        // System.out.println(tabbedPane.getSelectedComponent());
        // System.out.println(area);
        editor.addKeyListener(new compile_File());
            ...
            if (ret == JFileChooser.APPROVE_OPTION) {
                try {
                    ...
                    if (is_it_okay) {
                        ...
                        add(createTabbedPane(name, path, input));
                    } else {
    ...
    public void File_save_write(int tab) {
        try {
            ...
            JTextArea area = file_jtextarea_list.get(tab);
    ...
    public void File_save_as(int tab, String new_file) {
        try {
            if (File_save_check(tab, file_path_list.get(tab), new_file)) {
                // 해당 파일이 이미 존재함
                // 다이얼로그 출력
                JOptionPane.showMessageDialog(null, "이미 해당 파일이 존재합니다", "Already Existing File", JOptionPane.ERROR_MESSAGE);
            }else{
    ...
    class save_File_as implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (new_file == null) {
                // 아무것도 안 하고 그냥 리턴 → 다이얼로그 안 뜸
                return;
            }

            // 빈 문자열 입력 방지
            if (new_file.equals("")) {
                JOptionPane.showMessageDialog(null, "파일명을 입력해야 합니다!", "경고", JOptionPane.WARNING_MESSAGE);
                return;
            }
            
            if(!new_file.endsWith(".java")) {
            	new_file += ".java";
            }
    ...
    class close_File implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            file_jtextarea_list.remove(tab);
        }
    }
    ...
    class compile_File extends KeyAdapter implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            JTextArea file = file_jtextarea_list.get(tab);
            if (file == null) {
